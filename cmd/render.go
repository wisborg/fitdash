...
func bindRenderFlags(c *cobra.Command) {
	...
	f.StringVar(&renderOpts.theme, "theme", panel.DefaultTheme().Name,
		"colour palette -- \"night\" (default), \"dark\" or \"light\". night and dark are the same dashboard; they differ in "+
			"the basemap --basemap "+tilemap.LocalProvider+" draws for them. night draws it as linework: the roads, railways "+
			"and boundaries over black, with water as the only filled feature. dark fills the landuse surfaces as well, which "+
			"is more map and more of it competing with the route. Without a basemap the two differ only in slightly lighter "+
			"chrome under night")
...
func resolveBasemap(cmd *cobra.Command, theme panel.Theme, track *fitactivity.Track, labels tilemap.LabelFaces) (tilemap.Provider, error) {
	...
	if style == tilemap.LocalProvider {
		return resolveLocalBasemap(cmd, theme, track, labels)
	}
...
//
// A fraction rather than a size, for the same reason every other dimension in
// this program is one: a render at 4K and the same render at 1080p have to
// look like the same picture, and a font pinned to pixels would be half the
// size in one of them.
//
// The value is small on purpose. These are names on a background map, read
// beside readouts set twenty times larger, and a map label competing with the
// dashboard for attention is a map label in the way. 0.013 puts it at 14px on
// a 1080p frame, which is about where a printed map sets its minor place
